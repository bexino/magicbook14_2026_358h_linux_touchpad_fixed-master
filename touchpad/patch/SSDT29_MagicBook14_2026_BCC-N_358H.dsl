 ...
 *
 * Disassembly of SSDT29_original.aml
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00008B33 (35635)
 *     Revision         0x02
 *     Checksum         0x4D
 *     OEM ID           "HONOR"
...
DefinitionBlock ("", "SSDT", 2, "HONOR", "I2C_DEVT", 0x00001000)
{
    ...
    Scope (\_SB.PC00.I2C0)
    {
        ...
        {
            Name (HID2, Zero)
            Name (SBFB, Buffer (0x19)
            {
                /* 0000 */  0x8E, 0x14, 0x00, 0x02, 0x00, 0x01, 0x02, 0x00,  // ........
                /* 0008 */  0x00, 0x01, 0x06, 0x00, 0x80, 0x1A, 0x06, 0x00,  // ........
                /* 0010 */  0x00, 0x00, 0x4E, 0x55, 0x4C, 0x4C, 0x00, 0x79,  // ..NULL.y
                /* 0018 */  0x00                                             // .
            })
            Name (SBFI, Buffer (0x0B)
            {
                /* 0000 */  0x89, 0x06, 0x00, 0x15, 0x01, 0x00, 0x00, 0x00,  // ........
        ...
        Device (TPL1)
        {
            ...
            Name (SBFI, Buffer (0x0B)
            {
                /* 0000 */  0x89, 0x06, 0x00, 0x05, 0x01, 0x00, 0x00, 0x00,  // ........
                /* 0008 */  0x00, 0x79, 0x00                                 // .y.
            })
            CreateWordField (SBFB, 0x10, BADR)
            CreateDWordField (SBFB, 0x0C, SPED)
            ...
            Method (_INI, 0, NotSerialized)  // _INI: Initialize
            {
                ...
                If ((DerefOf (TPTD [Zero]) == 0x07))
                {
                    ...
                    ADBG (Concatenate ("TPLH PchI2cTouchPanelHidAddress", ToHexString (DerefOf (TDPH [Zero]))))
                    ADBG (Concatenate ("TPLB PchI2cTouchPanelBusAddress", ToHexString (DerefOf (TPTD [0x02]))))
    ...
    Scope (\_SB.PC00.I2C5)
    {
        ...
        Device (TPL1)
        {
            ...
            Method (_INI, 0, NotSerialized)  // _INI: Initialize
            {
                ...
                INT2 = INUM (DerefOf (TPDI [0x02]))
                ADBG (Concatenate ("TPLM PchI2cTouchPanelIrqMode", ToHexString (DerefOf (TPTD [0x02]))))
                If ((DerefOf (TPTD [One]) == Zero))
                {
                    SHPO (DerefOf (TPDI [0x02]), One)
                }
            ...
            Method (_DSM, 4, Serialized)  // _DSM: Device-Specific Method
            {
                ...
                If ((Arg0 == TP7G))
                {
                    Return (TP7D (Arg0, Arg1, Arg2, Arg3, SBFB, G_IN (DerefOf (TPDI [0x02]
                        ), Zero, One, Zero, Zero, Zero)))
                }
            ...
            Method (_STA, 0, NotSerialized)  // _STA: Status
            {
                ADBG (Concatenate ("TPLT 2PchI2cTouchPanelType", ToHexString (DerefOf (TPTD [Zero]))))
                ADBG (Concatenate ("TPLH 2PchI2cTouchPanelHidAddress", ToHexString (DerefOf (TDPH [Zero]))))
                ADBG (Concatenate ("TPLB 2PchI2cTouchPanelBusAddress", ToHexString (DerefOf (TPTD [0x02]))))
                ADBG (Concatenate ("TPLS 2PchI2cTouchPanelSpeed", ToHexString (DerefOf (TPTD [0x03]))))
                ADBG (Concatenate ("TPDI Touchpanel2Irq", ToHexString (DerefOf (TPDI [0x02]))))
                If (((DerefOf (TPTD [Zero]) != Zero) && (I2CN & 
                    0x02)))
                {
                    Return (0x0F)
            ...
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                If ((DerefOf (TPTD [One]) == Zero))
                {
                    Return (ConcatenateResTemplate (I2CM (I2CX, BADR, SPED), G_IN (DerefOf (TPDI [0x02]), 
                        Zero, One, Zero, Zero, Zero)))
                }
